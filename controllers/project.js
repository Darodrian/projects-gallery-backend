...
var controller = {
	...
	getProjects: function(req, res){
		Project.find({}).exec((err, projects) => {
			...
			if(!projects) return res.status(404).send({message: 'No hay pryectos para mostrar'});
	...
	updateProject: function(req, res){
		...
		Project.findByIdAndUpdate(projectId, update, {new:true}, (err, projectUpdated) =>{
			...
			if(!projectUpdated) return res.status(404).send({message: 'No existe proyecto'});
	...
	deleteProject: function(req, res){
		...
		Project.findByIdAndRemove(projectId, (err, projectDeleted) => {
			...
			if(!projectDeleted) return res.status(404).send({message: 'No existe proyecto'});
